...
ostream& operator<<(ostream& out,const Date& da)//----------重载输出流运算符，
{
    out<<da.year<<"-"<<da.month<<"-"<<da.day<<endl;
    return out;
}

istream& operator>>(istream& in,Date& d)//------------------重载输入流域算符
{
    in>>d.year>>d.month>>d.day;
    return in;
}

Date& Date::operator++()//-----------------------------------重载前自增运算符
{
    nextDay();
    return (*this);
}

Date Date::operator++(int)//---------------------------------重载后自增运算符
{
    Date d=(*this);
    nextDay();
    return (*this);
