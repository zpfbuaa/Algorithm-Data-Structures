...
class Date
{
    friend ostream& operator<<(ostream& out,const Date& d);//------友元函数，重载输出流运算符
    friend istream& operator>>(istream& in,Date& d);//-------------友元函数，重载输入流运算符
    public:
//        void print() const ;   //--------------------------------打印函数，将当前日期打印到控制台
    ...
    protected:
    private:
        bool checkDate();//--------------------工具函数，检查当前日期数据是否合法；
        //-------------------------------------如果不合法，则修改为默认的合法值，并返回false;否则返回true
