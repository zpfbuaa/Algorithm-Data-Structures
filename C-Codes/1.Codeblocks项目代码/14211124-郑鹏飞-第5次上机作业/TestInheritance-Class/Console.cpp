    ...
    while (choice!=0)
    {
        switch (choice)
        {
            ...
            case 3: //输出学生选课信息（暂时不包括成绩）
                cout<<*student;
                cout<<"绩点成绩为:"<<fixed<<setprecision(2)<<student->calcCredit()<<endl<<endl;
                break;
            case 4: //设置学生成绩
                setCourseScore();
                break;
...
{
    ocNum = 3;
    oc[0]=new ObligatoryCourse("高级语言程序设计-2", 3);
	oc[1]=new ObligatoryCourse("数学分析", 5);
	oc[2]=new ObligatoryCourse("大学物理", 4);

	ecNum = 2;
	ec[0]=new ElectiveCourse("音乐欣赏", 2);
	ec[1]=new ElectiveCourse("软件英语",2);
